...
"""
stt_service.py
- STT: Wav2Vec2로 한국어 음성 → 텍스트
- 발음 유사도: G2P → 자모 분해(종성 7자 중화) → 가중 레벤슈타인 → 0~100 점수
- 참조 문장 조회: Spring API 호출

SOLID/DDD 메모
- 외부 의존(HF 모델, G2P, Spring API)은 어댑터 함수로 감싸서 교체 용이.
- 도메인 로직(발음 유사도 계산)은 순수함수로 분리.
"""
...
# HF 한국어 STT 모델 (필요 시 교체 가능: 팩토리/환경변수로 분리)
...
def transcribe_wav(wav_path: str) -> str:
    """
    파일 경로의 WAV(또는 가용 포맷)를 로드 → 16kHz 모노로 리샘플 → 한국어 STT.
    반환: 소문자/공백 정리된 한글/영문 문자열.
    """
